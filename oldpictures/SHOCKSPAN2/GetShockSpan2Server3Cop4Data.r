# GetShockSpan2Server3Cop4Data.r
...
# P A R A M S
sPlotFile <- "shockspan2server3cop4.png"
...
fnSubset <- function(dfIn)  {subset(dfIn
                            , copies %in% c(4,5) & serverdefaultlife==30000)}
...
sTitleLine <-   (   ""
                %+% "Shocks freq=2yr dur=1yr span=2 serverdefaultlife=3yr "
                %+% " "
                %+% " "
                %+% "\n"
                %+% "\n(Copies=4, annual total auditing)"
                )
