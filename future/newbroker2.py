...
def fnRunEverything(gl, llsInstructions, nWaitMsec, nWaitHowMany):
    ...
    thrStart = CStartAllCases(gl, gl.nWaitMsec, gl.nWaitHowMany
                            , llsInstructions, )
    thrEnd = CEndAllCases(gl, gl.nWaitMsec, )
    gl.llsFullOutput = [["",""]]
    thrStart.start()
    thrEnd.start()
    ...
    return tWaitStats(slot=gl.nWaitedForSlot, done=gl.nWaitedForDone)
    ...
    def run(self):
        while (fnbWaitForOpening(self.gl, self.nWaitMsec, self.nWaitHowMany)
            ...
            with self.gl.lockJobList:
                ...
                sLogFilename = "foo" + ("%03d"%(fnIntPlease(nJob))) + ".log"        # TEMP
                proc = multiprocessing.Process(target=fntDoOneCase
                                , args=(lLines, qOut, sLogFilename)
                                )
...
class CEndAllCases(threading.Thread):
    ...
    #@ntracef("END")
    def __init__(self, gl, mynWaitMsec):
        threading.Thread.__init__(self)
        self.gl = gl
        self.nWaitMsec = mynWaitMsec
