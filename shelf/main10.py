...
def getCliArgsForEverythingElse():
    ...
    if dCliDict["nSimlength"]:     G.nSimlength = dCliDict["nSimlength"] * 1000
    if dCliDict["nRandomseed"]:    G.nRandomseed = dCliDict["nRandomseed"]

    if dCliDict["lBER"]:           G.lBER = dCliDict["lBER"]
    if dCliDict["lCopies"]:        G.lCopies = dCliDict["lCopies"]
    if dCliDict["sLogLevel"]:      G.sLogLevel = dCliDict["sLogLevel"]
    if dCliDict["sLogFile"]:       G.sLogFile = dCliDict["sLogFile"]
    if dCliDict["nDocSmall"]:      G.nDocSmall = dCliDict["nDocSmall"]
    if dCliDict["nDocLarge"]:      G.nDocLarge = dCliDict["nDocLarge"]
    if dCliDict["nDocSmallPct"]:   G.nDocSmall = dCliDict["nDocSmallPct"]
    if dCliDict["nDocPctSdev"]:    G.nDocPctSdev = dCliDict["nDocPctSdev"]
