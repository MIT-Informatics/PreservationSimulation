...
#        ,mean=mean(lost)
    ...
    summ.impact.before.copies <- arrange(summ.first 
                ,glitchfreq
                ,glitchmaxlife
#                ,glitchspan
                ,glitchimpact
    ...
    summ.impact.before.life <- arrange(summ.first
                ,glitchfreq
                ,glitchimpact
                ,glitchmaxlife
#                ,glitchspan
    ...
    summ.life.before.impact <- arrange(summ.first
                ,glitchfreq
                ,glitchmaxlife
                ,lifem
                ,glitchimpact
...
# also want 
# foo <- arrange(summidpct, glitchfreq, copies, lifem, impact)
#
    # How do we make this printing similar to the old style???
    sink("../hl/tabs/summ-glitch-3_reordering.txt")
...
#    colnames(summ.first) <- column.labels
    ...
    fnPageHeading()
    cat("\n\nSummary losses (midmeans), impact before copies\n\n")
    colnames(summ.impact.before.copies) <- column.shortlabels
    print(summ.impact.before.copies)

    fnPageHeading()
    cat("\n\nSummary losses (midmeans), impact before life\n\n")
    colnames(summ.impact.before.life) <- column.shortlabels
    print(summ.impact.before.life)

    fnPageHeading()
    cat("\n\nSummary losses (midmeans), life before impact \n\n")
    colnames(summ.life.before.impact) <- column.shortlabels
    print(summ.life.before.impact)
