...
#==========================================================================
    # It is ESSENTIAL that these three lists are maintained in the 
    #  SAME ORDER.  Otherwise, the column labels get attached to 
    #  the wrong variables, which makes them soggy and hard to read.  
...
#        ,mean=mean(lost)
    ...
    head.reorder1 <- "Summary losses (midmeans), reordering 1: freq duration impact life copies svrhl"
    ...
    head.reorder2 <- "Summary losses (midmeans), reordering 2: freq impact duration life copies svrhl "
    ...
    head.reorder3 <- "Summary losses (midmeans), reordering 3: svrhl freq copies life duration impact life "
    ...
    head.reorder4 <- "Summary losses (midmeans), reordering 4: svrhl freq glitchdur copies life impact "
...
#                ,serverdefaultlife     # confounded with glitchfreq
#                ,shockmaxlife
#                ,shockimpact
                ,copies
                ,lifem
    ...
    sink(paste(dirname(sOutputFilename), "/summ-shockvslife-4_reordering.txt", 
        sep=""))
...
{
    datn <- data.frame(dat$glitchfreq,dat$glitchimpact,dat$glitchmaxlife,dat$lifem,dat$copies,dat$lost,dat$docsize,dat$serverdefaultlife)
    return(datn)
}

# N O T   U S E D 
# This is a leftover from the old days.  I want to do this, but have 
#  to recode it
...
# 20170522  RBL Special version to run comparisons of bad glitches versus
#                servers with finite lifetimes.  
#                Add serverdefaultlife to columns to be grouped.
#                Try a number of different reorderings.
# 20170525  RBL Add more reordering to try to get relevant numbers
#                to be adjacent for easy reading.  
# 
# 
