...
class CAudit2(object):
    ...
    def mAuditCycle(self,mynCycleInterval):
        ...
        while True:
            ...
            lg.logInfo("AUDIT2","begin cycle t|%10.3f| auditid|%s| cycle|%s| cli|%s| coll|%s|" % (G.env.now,self.ID,self.nNumberOfCycles,self.sClientID,self.sCollectionID))
            self.nRepairsThisCycle = 0
            ...
            lg.logInfo("AUDIT2","end cycle   t|%10.3f| auditid|%s| cycle|%s| cli|%s| coll|%s| repairs|%d| total|%d| perms|%d| forensics|%d|" % (G.env.now,self.ID,self.nNumberOfCycles,self.sClientID,self.sCollectionID,self.nRepairsThisCycle,self.nRepairsTotal,self.nPermanentLosses,self.nForensicsRequired))
    ...
    def mAuditCollection(self,mynCycleInterval,mynSegments,mysCollectionID,myeCallerSyncEvent):
        ...
        lg.logInfo("AUDIT2","end colln   t|%10.3f| auditid|%s| cycle|%s| cli|%s| coll|%s| repairs|%d| total|%d| perms|%d| forensics|%d| duration|%9.3f|" % (G.env.now,self.ID,self.nNumberOfCycles,self.sClientID,self.sCollectionID,self.nRepairsThisCycle,self.nRepairsTotal,self.nPermanentLosses,self.nForensicsRequired,self.fTimeCycleLength))
    ...
    def mAuditSegment(self,mynThisSegment,mylDocs,mysCollectionID,myeCallerSyncEvent):
        ...
        '''
        lg.logInfo("AUDIT2","begin segmt t|%10.3f| auditid|%s| cycle|%s| seg|%s| cli|%s| coll|%s|" % (G.env.now,self.ID,self.nNumberOfCycles,mynThisSegment,self.sClientID,self.sCollectionID))
                ...
                for sDocID in self.lDocsThisSegment:
                    ...
                    if self.mIsDocumentLost(sDocID):
                        ...
                        if fTransferTime:
                            ...
                            lg.logInfo("AUDIT2","doc lost    t|%10.3f| auditid|%s| cycle|%s| seg|%s| cli|%s| coll|%s| doc|%s| svr|%s|" % (G.env.now,self.ID,self.nNumberOfCycles,mynThisSegment,self.sClientID,self.sCollectionID,sDocID,sServerID))
...
class CAudit_Systematic(CAudit2):
    ''' \
