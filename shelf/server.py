...
# S h e l f . m A g e _ s e c t o r 
    ...
    def mAge_sector(self,mynLifeParam):
        ...
        while self.bAlive:
            ...
            TRC.tracef(3,"FAIL","proc t|%d| sector failure lib|%s| qual|%d| shelf|%s| doc|%s|" % (G.env.now,self.sLibID,G.dID2Library[self.sLibID].nQual,self.ID,sDocVictimID))
...
# S h e l f . m A g e _ s h e l f 
    ...
    def mAge_shelf(self,mynLifeParam):
        ...
        self.bAlive = False
        TRC.tracef(3,"FAIL","proc t|%d| shelf failure lib|%s| qual|%d| shelf|%s| docs|%d|" % (G.env.now,self.sLibID,G.dID2Library[self.sLibID].nQual,self.ID,len(templDocuments)))
