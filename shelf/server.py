...
# S h e l f . m A g e _ s e c t o r 
    ...
    def mAge_sector(self,mynLifeParam):
        ...
        while self.bAlive:
            ...
            logInfo("SERVER","small error time|%s| server|%s| shelf|%s| hidden failure in doc|%s|" % (G.env.now,self.sServerID,self.ID,sDocVictimID))
...
# S h e l f . m A g e _ s h e l f 
    ...
    def mAge_shelf(self,mynLifeParam):
        ...
        logInfo("SERVER","storage shelf failed time|%d| server|%s| shelf|%s| lost |%d| docs" % (G.env.now,self.sServerID,self.ID,len(templDocuments)))
