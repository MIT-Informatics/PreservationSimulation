...
then 
    ...
    #Bzzzt: for shaping runs, permit lifetime 0001
...
do 
    ...
    for copies in 01 02 03 04 05 06 07 08 09 10      # for shaping runs
    ...
    do
        ...
        then
            ...
            echo "python runsequence.py $instructionfilename --ncores=$usencores"
            if [ -n "$3" ]
            then
                echo "Testing only..."
            else
                # Finally, execute something.
                python runsequence.py $instructionfilename --ncores=$usencores
            fi
        fi
    done
