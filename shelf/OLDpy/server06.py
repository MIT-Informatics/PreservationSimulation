...
# S h e l f . m A c c e p t D o c u m e n t 
    ...
    def mAcceptDocument(self,mysDocID,mynDocSize,mysClientID):
        ''' Shelf.mAcceptDocument
            If there is room on the shelf, add the document and return True.
            If not, return False.
        '''
        if self.nFreeSpace >= mynDocSize:
...
# S h e l f . m A g e _ s h e l f 
    ...
    def mAge_shelf(self,mynLifeParam):
        ...
        logInfo("SERVER","storage shelf failed time|%d| server|%s| shelf|%s| lost |%d| docs" % (G.env.now,self.sServerID,self.ID,len(templCopyIDs)))
