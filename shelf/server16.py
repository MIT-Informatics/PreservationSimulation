...
class CShelf(object):
    ...
    def __init__(self,mysServerID,mynQual,mynCapacity):
        ...
        self.sSectorLifeID = CLifetime(self.ID,self.nSectorLife).ID
...
# S h e l f . m A g e _ s e c t o r 
    ...
    def mAge_sector(self):
        ...
        while self.bAlive:
            ...
            fSectorLifetime = util.makeexpo(fLifeParam)
            ...
            if sCopyVictimID:               # Hidden error in victim doc.
                ...
                if self.nConsecutiveMisses > 0:
                    logInfo("SERVER","small error t|%6.0f| svr|%s| shelf|%s| consecutive misses|%d|" % (G.env.now,self.sServerID,self.ID,self.nConsecutiveMisses))
                self.nConsecutiveMisses = 0
                logInfo("SERVER","small error t|%6.0f| svr|%s| shelf|%s| hidden failure in copy|%s| doc|%s|" % (G.env.now,self.sServerID,self.ID,sCopyVictimID,sDocID))
...
class CLifetime(object):
    ...
    @tracef("LIFE")
    def __init__(self,mysShelfID,mynLifetime):
        self.fOriginalLifetime = float(mynLifetime)
        self.fCurrentLifetime = self.fOriginalLifetime
        self.nErrorInjectionRate = -1           # Never.
        self.fErrorInjectionTime = float(0)     # Not yet.
        self.nReductionPercentage = 0 #(int from G somewhere: 5, 10, 20, etc.)
        self.fDecayHalflife = 0 #(again from G: 1..N, where 1 decays quickly, N slowly)
        self.sShelfID = mysShelfID
        self.ID = "life" + self.sShelfID

    @tracef("LIFE")
    def mSetInjectionRate(self,mynInjectionRate):
        self.nErrorInjectionRate = mynInjectionRate

    @tracef("LIFE")
    def mGetInjectionRate(self):
        return self.nErrorInjectionRate

    @tracef("LIFE")
    def mInjectError(self,mynReduction,mynDecayHalflife,myfNow):
        '''\
        At some random time, decrease lifetime by some amount, percentage.
        ...
        self.fErrorInjectionTime = float(G.env.now)
        return fDecayRate

    @tracef("LIFE")
    def mfCalcCurrentLifetime(self,myfNow):
        fNow = G.env.now
        fExponentialDecay = exp(- (fNow - fErrorInjectionTime) / self.fDecayRate)
        fLifetimeLossPct = self.nReductionPercentage * (1 - fExponentialDecay)
        self.fCurrentLifetime = self.fOriginalLifetime * (fLifetimeLossPct/100.0) 
...
#               Begin work on correlated failures.
#
#
