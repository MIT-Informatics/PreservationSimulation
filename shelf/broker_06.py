...
class CG(object):
    ...
    sExtractCmdTemplate = 'python extractvalues.py --header --separator=" " q3-extractinstructions-01.txt {sFamilyDir}/{sSpecificDir}/log/{sShelfLogFileName}.log > {sFamilyDir}/{sSpecificDir}/ext/{sShelfLogFileName}.ext'
...
def main():
    ...
    Don't want to disable the timeout completely, so collect the entire
     ...
     citizen and what does it get you.  
    '''

    ldAllInstructions = list(itCurrentSet)
    NTRC.ntracef(0,"MAIN","proc main nInstructionsqueried|{}|".format(len(ldAllInstructions)))

    for dInstruction in ldAllInstructions: 
