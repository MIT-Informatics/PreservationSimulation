...
class CG(object):
    ...
    sExtractCmdTemplate = 'python extractsomethingorother {sFamilyDir}/{sSpecificDir}/log/{sShelfLogFileName}.log > {sFamilyDir}/{sSpecificDir}/dat/{sShelfLogFileName}.ext'
    ...
    sActorCmdTemplate = 'python listactor.py {sActorCmdFileName} > {sFamilyDir}/{sSpecificDir}/log/{sShelfLogFileName}_actor.log 2>&1 &'
    sActorCmd = None
...
class CFormat(object):
    ...
    def fndFormatQuery(self, mydCli):
        ...
        for sAttrib,sValue in mydCli.items():
            NTRC.tracef(3, "FMT", "proc FormatQuery item key|%s| val|%s|" % (sAttrib, sValue))
    ...
    for dInstruction in itCurrentSet: 
        ...
        if bContinue:
            ...
            with open(g.sActorCmdFileName, 'w') as fhActorCmdFile:
                print >> fhActorCmdFile, cFmt.fnsMaybeTest(sRunCommand)
                print >> fhActorCmdFile, cFmt.fnsMaybeTest(sExtractCommand)
