...
class CG(object):
    ...
    # TEMP TEMP TEMP
    sShelfRunCmdTemplate = "echo {sShelfLogFileName}"
    sExtractCmdTemplate = 'python test/fib.py 42'
    sActorCmdFileTemplate = './{sShelfLogFileName}.cmds'
    sActorCmdTemplate = 'python listactor.py {sActorCmdFileName} > broker_01test_{sShelfLogFileName}_output.txt &'
    # END
...
# C F o r m a t . f n d F o r m a t Q u e r y 
    @ntracef("FMT")
...
# C F o r m a t . f n l F o r m a t C o m m a n d s F o r I n s t r u c t i o n s 
    @ntracef("FMT")
    def fnlFormatCommandsForInstructions(self, mydInstruction):
        """
        oCmd = CCommand()
        ...
        """
        cFmt = CFormat()
        sRunCommand = cFmt.msGentlyFormat(g.sShelfRunCmdTemplate, mydInstruction)
        sExtractCommand = cFmt.msGentlyFormat(g.sExtractCmdTemplate, mydInstruction)
        return [sRunCommand, sExtractCommand]

# C F o r m a t . f n s F o r m a t C o m m a n d F o r A c t o r 
    @ntracef("FMT")
    def fnsFormatCommandForActor(self, mydInstruction):
        cFmt = CFormat()
        g.sActorCmdFileName = cFmt.msGentlyFormat(g.sActorCmdFileTemplate, mydInstruction)
        sActorCommand = cFmt.msGentlyFormat(g.sActorCmdTemplate, mydInstruction)
        return sActorCommand
...
def main():
    ...
    for dInstruction in itCurrentSet: 
        ...
        if bContinue:
            # Make list of commands for actor base on instructions.            
            g.sShelfLogFileName = cFmt.msGentlyFormat(g.sShelfLogFileTemplate, dInstruction)
            lCommands = cFmt.fnlFormatCommandsForInstructions(dInstruction)
            
            # Make instruction file for the actor and launch it.
            g.sActorCommand = cFmt.fnsFormatCommandForActor(dInstruction)
            NTRC.ntracef(0,"MAIN","proc main cmds\n1|%s|\n2|%s|\n" % (lCommands, 
g.sActorCommand))
            
            with open(g.sActorCmdFileName, 'w') as fhActorCmdFile:
                for sLine in lCommands:
                    print >> fhActorCmdFile, sLine
