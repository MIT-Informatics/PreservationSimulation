# GetShockSpan2Server5Seg4Data.r
...
# P A R A M S
sPlotFile <- "shockspan2server5Seg4.png"
...
fnSubset <- function(dfIn)  {subset(dfIn, serverdefaultlife==50000
                                    & copies>=6
                                    & auditsegments==4
                                    )}
...
sTitleLine <-   (   ""
                %+% "Shocks: span=2; ServerDefaultHalflife=5yrs; "
                %+% "samples=5000 "
                %+% " "
                 %+% "\n"
                %+% "\n(Copies=6,7; annual systematic auditing in 4 segments)"
                )
