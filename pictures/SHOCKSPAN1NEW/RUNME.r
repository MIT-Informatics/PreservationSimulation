# where to run?
# setwd("C:/cygwin64/home/landau/working/PreservationSimulation/pictures/SHOCKSPAN1NEW")
debugprint<-0
#source("./GetShockSpan1Server1Data.r")
#source("./GetShockSpan1Server2Data.r")
source("./GetShockSpan1Server3Seg1Data.r")
source("./GetShockSpan1Server3Seg4Data.r")
source("./GetShockSpan1Server3Seg10Data.r")
#source("./GetShockSpan1Server4Data.r")
#source("./GetShockSpan1Server4Seg1Data.r")
#source("./GetShockSpan1Server4Seg4Data.r")
source("./GetShockSpan1Server4Seg10Data.r")
