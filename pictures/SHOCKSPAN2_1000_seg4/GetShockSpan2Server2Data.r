...
sTitleLine <-   (   ""
                %+% "Shocks frequency=2yr duration=1yr span=2 serverdefaultlife=2yr "
                %+% "\n(various copies) (sample size=1000) "
                %+% " "
                %+% "\n"
                %+% "\n(annual total auditing, quarterly segments)"
                )
...
summarize(losspct=round(mean(lost/docstotal)*100.0, 3), n=n()) %>%
fnSubset()
trows <- newdat
