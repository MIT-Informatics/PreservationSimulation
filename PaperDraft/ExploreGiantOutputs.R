...
sims.df.ls = lapply( filesList, 
      function(x)read.table("GiantOutput_freq1yr_20170717_doc50_00.txt",
...
resultVarNames <- c("docslost","docsrepairedmajority","docsrepairedminority","lost","deadserversactive","deadserversall","docsokay","auditmajority","auditminority","auditlost","auditrepairs")
...
gp<-ggplot(res, aes(x=copies, y=mnlossfrac)) +
  geom_point(shape=1) +    
...
gp<-ggplot(res, aes(x=copies, y=mnlossfrac)) +
  geom_point(shape=1) +    
  geom_smooth(method = lm) + facet_grid(~shockimpact~nshocks)
plot(gp)

# pairwise comparison plot of multiple variables -- just for fun
library(GGally)
gp<-ggpairs(res,columns=c("copies","mnlossfrac","shockimpact","nshocks"))
