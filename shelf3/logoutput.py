...
def logSetConfig(mysLogLevel,mysLogFile):
    ...
    if sLogLevel not in lLogLevels:
        NTRC.ntracef(0,"LGOU","ERROR unrecognized logging level|%s|" % (mysLogLevel))
        sLogLevel = "NOTSET"

    # Set the logging level for this session.
    NTRC.ntracef(3,"LGOU","proc sLogLevel|%s|"%(sLogLevel))
    ...
    NTRC.ntracef(3,"LGOU","proc set log handler mysLogFile|%s|" % (mysLogFile))
