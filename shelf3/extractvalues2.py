...
def fnldParseInput(mysFilename):
    ...
    with open(mysFilename,"rb") as fhInfile:
        ...
        NTRC.ntracef(3,"INPT","proc ParseInput template|%s|" % (lTemplate))
        ...
        NTRC.ntracef(5,"INPT","proc ParseInput lRowDicts all|%s|" % (lRowDicts))
...
def main(mysInstructionsFileName,mysLogFileName):
    ...
    with open(mysLogFileName,"r") as fhLogFile:
        ...
        NTRC.ntracef(3,"MN2","proc lLinesSelectedRaw len|%s| all|%s|" % (len(lLinesSelectedRaw),lLinesSelectedRaw))

    # Eliminate duplicates.  Should not be any if the lineregexes are 
    #  specific enough.  
    lLinesSelected = list(set(lLinesSelectedRaw))
    NTRC.ntracef(5,"MN3","proc lLinesSelected len|%s| all|%s|" % (len(lLinesSelected),lLinesSelected))

    # Extract variable value from each matching line.
    # List of lines selected is actually a list of triples.
    lResults = map( lambda omatch_sLine_sVarname: 
                fntMatchValue(omatch_sLine_sVarname[1], g.dVars[omatch_sLine_sVarname[2]])
                , lLinesSelected )
...
def fntDoesLineMatchThisVar(mysLine, mynLineNr, mysVarname):
    ...
    NTRC.ntracef(5,"MTLN","proc MatchLine try regex|%s| var|%s| nr|%s| line|%s| match|%s|" % (sLineregex,mysVarname,mynLineNr,mysLine,oMatch))
    if oMatch:
        NTRC.ntracef(3,"LINE","proc MatchLine found line|%s|=|%s| var|%s| regex|%s|" % (mynLineNr,mysLine,mysVarname,sLineregex))
...
def fntMatchValue(mysLine,mydVar):
    ...
    NTRC.ntracef(5,"MCHV","proc MatchValue matching word var|%s| word|%s| valueregex|%s| matchobj|%s|" % (sVarname,sWord,sValueregex,oMatch))
    if oMatch:
        # Word matches the valueregex.  Save the value.
        sValue = oMatch.group(1)
        NTRC.ntracef(3,"MCHV","proc addvalue name|%s| val|%s|" % (sVarname,sValue))
    else:
...
if "__main__" == __name__:
    ...
    timestop = clock()
#    NTRC.ntracef(0,"MAIN","proc cputime|%s|" % (timestop-timestart))
