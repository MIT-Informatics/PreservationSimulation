    ...
    with open(mysFilename,"rb") as fhInfile:
        ...
        for sLine in lLines[:]:
            if re.match("^ *#",sLine) or re.match("^ *$",sLine.rstrip()):
                lLines.remove(sLine)
                NTRC.ntrace(3,"proc ParseInput remove comment or blank line |%s|" % (sLine.strip()))
        # The first non-blank, non-comment line is the command template.
        sCmd = lLines.pop(0).strip()
        NTRC.ntrace(3,"proc ParseInput command line|%s|" % (sCmd))
        ...
        for dRow in lRowDicts:
            ...
            NTRC.ntrace(5,"proc fndParseInput dRow|%s| dNewRow|%s| lParams|%s|" % (dRow,dNewRow,lParams))
    return (sCmd,lParams)
...
def fnWaitForOpening(mynProcessMax,mysProcessName,mynWaitTime,mynWaitLimit):
    ...
    for idx in range(mynWaitLimit):
        ...
        nResult = int(sResult)
        NTRC.ntrace(3,"proc WaitForOpening1 idx|%d| cmd|%s| result|%s|" % (idx,sFullCmd,sResult))
        if nResult < mynProcessMax + 1 if (mysProcessName.find("python") >= 0) else 0:
            break
        NTRC.ntrace(3,"proc WaitForOpening2 sleep and do again idx|%d| nResult|%d|" % (idx,nResult))
        sleep(mynWaitTime)
...
#    fnMaybeOverride("nGroupTimer",dCliDict,g)
    ...
    for idx in range(len(lParams)):
        ...
        if bContinue:
            ...
            print('-----------------')
            NTRC.ntrace(0,sFullCmd)
            print('-----------------')
            lResult = cCommand.doCmdLst(sFullCmd)
            for sResult in lResult:
                NTRC.ntrace(0,sResult)
            print('-----------------')
            sleep(g.nPoliteTimer)
        else:
            NTRC.ntrace(0,"OOPS, Stuck!  Too many python processes running forever.")
            break
