...
class CShelf(object):
    getID = itertools.count(1).__next__
...
# S h e l f . m A d d D o c u m e n t 
    ...
    def mAddDocument(self, mysDocID, mysClientID):
        ...
        sCopyID = cCopy.ID
        NTRC.ntracef(3,"SHLF","proc mAddDocument made copy|%s| of doc|%s| "
            "from client|%s|" 
...
#        sServerID = self.sServerID
        ...
        NTRC.ntracef(5,"SHLF","proc mAddDocument add doc|%s| to shelf|%s| "
            "size|%d| remaining|%d|" 
...
# S h e l f . m A g e _ s e c t o r 
    ...
    def mAge_sector(self):
        ...
        while self.bAlive:
            ...
            NTRC.ntracef(3, "SHLF", "proc mAge_sector time|%d| shelf|%s| "
            ...
            if sCopyVictimID:               # Hidden error in victim doc.
                ...
                NTRC.ntracef(3, "FAIL", "proc t|%d| sector failure server|%s| "
                ...
                NTRC.ntracef(3, "SHLF", "proc mAge_sector shelf|%s| "
...
# S h e l f . m S e l e c t V i c t i m C o p y  
    ...
    def mSelectVictimCopy(self, mynErrorSize):
        ...
        nLoc = 0
        NTRC.ntracef(5, "SHLF", "proc SelectVictimCopy0 wherehit spot|%s| "
        ...
        if nRandomSpot <= self.nHiWater:
            ...
            nDist = (nLen + 1) // 2
            nLoc = nDist
            NTRC.ntracef(5, "SHLF", "proc SelectVictimCopy0 searchsetup len|%s| "
                "loc|%s| dist|%s|" 
                % (nLen, nLoc, nDist))
            while 1:
                if nLoc <= 0: nLoc = 1
                ...
                if nRandomSpot <= nTop:
                    ...
                    if nRandomSpot >= nBottom:
                        ...
                        NTRC.ntracef(5, "SHLF", "proc SelectVictimCopy5D "
                        ...
                        if sCopyID in self.lCopyIDs:
                            sVictimID = sCopyID
                            NTRC.ntracef(3, "SHLF", "proc mSelectVictimCopy "
                            ...
                            sVictimID = None
                            NTRC.ntracef(5, "SHLF", "proc SelectVictimCopy2D "
                        ...
                        nLoc -= nDist
                        NTRC.ntracef(5, "SHLF", "proc SelectVictimCopy3D "
                    ...
                    if nRandomSpot <= self.lCopyTops[nLoc+1]:
                        ...
                        sCopyID = self.lCopyIDsComplete[nLoc+1-1]
                        NTRC.ntracef(5, "SHLF", "proc SelectVictimCopy5U "
                            "found victim id|%s| at spot|%s| in[%s,%s]| doc|%s|" 
                            % (sCopyID, nRandomSpot, nBottom, nTop, sDocID))
                        # Is this slot still occupied by a live copy?
                        if sCopyID in self.lCopyIDs:
                            sVictimID = sCopyID
                            NTRC.ntracef(3, "SHLF", "proc mSelectVictimCopy NEWU "
                            ...
                            sVictimID = None
                            NTRC.ntracef(5, "SHLF", "proc SelectVictimCopy2U "
                                "no longer valid copyid|%s| docid|%s|" 
                                % (sCopyID, sDocID))
                            self.nMultipleHits += 1
                        break
                        ...
                        nLoc += nDist
                        NTRC.ntracef(5, "SHLF", "proc SelectVictimCopy3U up   "
                            "spot|%s| intvl|[%s,%s| newloc|%s| newdist|%s|" 
                            % (nRandomSpot, nBottom, nTop, nLoc, nDist))

        else:   # Outside hiwater area, just count as a miss.
            NTRC.ntracef(3, "SHLF", "proc mSelectVictimCopy shelf|%s| spot|%d| "
...
# S h e l f . m D e s t r o y C o p y  
    ...
    def mDestroyCopy(self,mysCopyID):
        try:
            ...
            NTRC.ntracef(0, "SHLF", "BUGCHECK copyID not found for removal|%s|" 
                % (mysCopyID))
            return False
        ...
        NTRC.ntracef(3, "SHLF", "proc mDestroyCopy remove doc|%s| copy|%s| "
...
# BZZZT: This isn't used anymore.  We now model server failures with glitches.
    ...
    def mAge_shelf(self, mynLifeParam):
        ...
        NTRC.ntracef(3, "SHLF", "proc mAge_shelf  time|%d| shelf|%s| "
        ...
        NTRC.ntracef(3, "SHLF", "proc mAge_shelf  time|%d| shelf|%s| shelf_error" 
            % (G.env.now,self.ID))
        ...
        NTRC.ntracef(5, "SHLF", "proc mAge_shelf kill contents ldocs|%s| "
            "lcopies|%s|" 
        ...
        NTRC.ntracef(3, "FAIL", "proc t|%d| shelf failure server|%s| qual|%d| "
            "shelf|%s| docs|%d|" 
...
# 20180531  RBL Really update to use NTRC, ntrace, ntracef.
